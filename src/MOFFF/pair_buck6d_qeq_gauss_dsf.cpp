...
#include "pair_buck6d_qeq_gauss_dsf.h"
#include <cmath>
#include <cstring>
#include "atom.h"
#include "comm.h"
...
PairBuck6dQEqGaussDSF::PairBuck6dQEqGaussDSF(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 1;
  writedata = 1;
}
...
PairBuck6dQEqGaussDSF::~PairBuck6dQEqGaussDSF()
{
  if (copymode) return;
...
void PairBuck6dQEqGaussDSF::compute(int eflag, int vflag)
{
  ...
  double *chi = qeqfix->get_chi();
  double *eta = qeqfix->get_eta();
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    eself = chi[itype]*q[i]+0.5*(q[i]*q[i]*eta[itype]);
...
void PairBuck6dQEqGaussDSF::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBuck6dQEqGaussDSF::settings(int narg, char **arg)
{
...
void PairBuck6dQEqGaussDSF::coeff(int narg, char **arg)
{
  if (narg < 7 || narg > 8)
...
void PairBuck6dQEqGaussDSF::init_style()
{
  if (!atom->q_flag)
    error->all(FLERR,"Pair style buck6d/qeq/dsf requires atom attribute q");

  neighbor->request(this,instance_me);
  qeqfix = (FixQEqGauss*) modify->get_fix_by_style("^qeq/gauss").front();
  cut_coul = qeqfix->get_cutoff();
...
double PairBuck6dQEqGaussDSF::init_one(int i, int j)
{
...
void PairBuck6dQEqGaussDSF::write_restart(FILE *fp)
{
...
void PairBuck6dQEqGaussDSF::read_restart(FILE *fp)
{
...
void PairBuck6dQEqGaussDSF::write_restart_settings(FILE *fp)
{
...
void PairBuck6dQEqGaussDSF::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairBuck6dQEqGaussDSF::write_data(FILE *fp)
{
...
void PairBuck6dQEqGaussDSF::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairBuck6dQEqGaussDSF::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairBuck6dQEqGaussDSF::extract(const char *str, int &dim)
{
  dim = 2;
