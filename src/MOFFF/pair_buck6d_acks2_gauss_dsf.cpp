...
#include "pair_buck6d_acks2_gauss_dsf.h"
#include <cmath>
#include <cstring>
#include "atom.h"
#include "comm.h"
...
PairBuck6dACKS2GaussDSF::PairBuck6dACKS2GaussDSF(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 1;
  writedata = 1;
}
...
PairBuck6dACKS2GaussDSF::~PairBuck6dACKS2GaussDSF()
{
  if (copymode) return;
...
void PairBuck6dACKS2GaussDSF::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairBuck6dACKS2GaussDSF::settings(int narg, char **arg)
{
...
void PairBuck6dACKS2GaussDSF::coeff(int narg, char **arg)
{
  if (narg < 7 || narg > 8)
...
void PairBuck6dACKS2GaussDSF::init_style()
{
  if (!atom->q_flag)
    error->all(FLERR,"Pair style buck6d/acks2/dsf requires atom attribute q");

  neighbor->request(this,instance_me);
  acks2fix = (FixACKS2Gauss*) modify->get_fix_by_style("^acks2/gauss").front();
  cut_coul = acks2fix->get_cutoff();
...
double PairBuck6dACKS2GaussDSF::init_one(int i, int j)
{
...
void PairBuck6dACKS2GaussDSF::write_restart(FILE *fp)
{
...
void PairBuck6dACKS2GaussDSF::read_restart(FILE *fp)
{
...
void PairBuck6dACKS2GaussDSF::write_restart_settings(FILE *fp)
{
...
void PairBuck6dACKS2GaussDSF::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairBuck6dACKS2GaussDSF::write_data(FILE *fp)
{
...
void PairBuck6dACKS2GaussDSF::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairBuck6dACKS2GaussDSF::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairBuck6dACKS2GaussDSF::extract(const char *str, int &dim)
{
  dim = 2;
