...
FixACKS2Gauss::FixACKS2Gauss(LAMMPS *lmp, int narg, char **arg) :
  FixQEqGauss(lmp, narg, arg), Xij_file(nullptr)
{
  X_diag = nullptr;
  Xdia_inv = nullptr;
  ...
  Xij_file = utils::strdup(arg[8]);
  read_Xij_file(Xij_file);
}
...
void FixACKS2Gauss::read_Xij_file(char *arg)
{
  ...
  if (comm->me == 0) {
    ...
    try {
      ...
      while((line = reader.next_line())) {
        ...
        for (int i = 0; i <= 3; i++)
          Xij[itype][jtype][i] = Xij[jtype][itype][i] = values.next_double();
      }
