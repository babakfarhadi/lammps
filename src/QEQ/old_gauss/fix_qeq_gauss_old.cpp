...
void FixQEqGauss::compute_H()
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      r = sqrt(r_sqr);
      H.jlist[m_fill] = j;
      H.val[m_fill] = calculate_H_dsf(zei, zej, r);
      m_fill++;
    }
  ...
  // DEBUG BABAK
  printf("DEBUG: inum: %1d gnum: %1d nlocal: %1d nghost: %1d cutoff: %12.6f\n", list->inum, list->gnum, atom->nlocal, atom->nghost, sqrt(cutoff_sq));
  if (m_fill >= H.m) {
    char str[128];
...
double FixQEqGauss::calculate_H_dsf(double zei, double zej, double r)
{
  ...
  // DEBUG BABAK
  printf("DEBUG sigi: %10.5f, sigj: %10.5f, r: %10.5f, etmp: %10.5f, Jij: %10.5f\n", sigi, sigj, r, etmp3, 0.5*qqrd2e*etmp3);
